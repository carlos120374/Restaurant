...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { address: @local.address, description: @local.description, distrit: @local.distrit, user_id: @local.user_id }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { address: @local.address, description: @local.description, distrit: @local.distrit, user_id: @local.user_id }
    assert_redirected_to local_path(assigns(:local))
  end
